...
const JurnalDetailPage = () => {
  ...
  const renderStars = (rating, ranks = []) => {
    ...
    // Cek apakah ada Q1–Q4 di ranks (baik dalam bentuk object ataupun string)
    const isHighRank = Array.isArray(ranks)
      ? ranks.some(rank =>
        typeof rank === "string"
          ? ["Q1", "Q2", "Q3", "Q4"].includes(rank.toUpperCase())
          : ["Q1", "Q2", "Q3", "Q4"].includes(rank.name?.toUpperCase())
      )
      : false;
          ...
          <div className="max-w-5xl mx-auto px-8 mb-12 space-y-4 bg-white p-6 rounded-lg shadow">
            ...
            <div className="info-row flex items-center gap-2">
              ...
              <span className="info-value flex items-center gap-1">
                : {renderStars(data?.rating_avg || 0, data?.ranks || [])}
              </span>
            ...
            <p className="info-row">
              ...
              <span className="info-value">
                : <a href={data?.url} target="_blank" rel="noopener noreferrer" className="underline">{data?.url}</a>
              </span>
            </p>
