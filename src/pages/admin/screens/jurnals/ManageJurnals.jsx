...
import { deleteJurnal, exportJurnalCSV, getAllJurnals } from "../../../../services/index/jurnals";
...
const ManageJurnals = () => {
  ...
  const exportCSV = async () => {
    try {
      // Fetch the CSV from the backend, passing the token
      const response = await exportJurnalCSV(userState.userInfo.token);
  ...
  return (
    <>
      <div className="mb-4 flex justify-end">
        <button
          className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-base font-semibold rounded-lg transition duration-200 flex items-center"
          onClick={exportCSV} // OnClick calls exportCSV function
        >
          <FiFileText className="mr-2" /> {/* CSV icon with margin-right */}
          Export CSV
        </button>
      </div>
